startTime <- Sys.time()
cat(paste0("> Rscript coexpr_dist_v2_4curves_tmp.R\n"))
...
for(i_fam in all_familyData) {
  ...
  if(buildTable) {
    ...
    outFile <- file.path(outFold, paste0(i_fam, "_family_dist_coexpr_DT.Rdata"))
    # outFile <- "/media/electron/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_TopDom/COEXPR_DIST_v2/TCGAcrc_msi_mss_hgnc/hgnc_family_family_dist_coexpr_DT.Rdata"
