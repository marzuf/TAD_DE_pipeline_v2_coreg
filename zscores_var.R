startTime <- Sys.time()
cat(paste0("> Rscript zscores_var.R\n"))
...
# Rscript zscores_var.R
# Rscript zscores_var.R TCGAcrc_msi_mss
#  Rscript zscores_var.R <dataset> 
...
#outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "ZSCORES_VAR",  paste0(curr_dataset, "_nTop", nTopTADs, "_", familyData))
outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "ZSCORES_VAR",  paste0(curr_dataset))
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, paste0("zscores_var.txt"))  
...
all_z_scores <- foreach(i_tad = seq_len(nrow(TADpos_DT))) %dopar% {  
  ...
  log2fpkm_tad <- log2(as.numeric(fpkmDT[tad_genes,])+0.0001)
  log2fpkm_adjTad <- log2(as.numeric(fpkmDT[adj_tad_genes,])+0.0001)
  
  z_scores_TAD <- as.numeric(scale(log2fpkm_tad, scale = T, center=T))
  z_scores_adjTAD <- as.numeric(scale(log2fpkm_adjTad, scale = T, center=T))

  list(z_scores_TAD = z_scores_TAD,
       z_scores_adjTAD = z_scores_adjTAD
       )  
}

names(all_z_scores) <- TADpos_DT$region

outFile <- file.path(outFold, "all_z_scores.Rdata")
save(all_z_scores, file = outFile)
cat(paste0("... written: ", outFile, "\n"))

# load("/media/electron/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_TopDom/ZSCORES_VAR/TCGAcrc_msi_mss_nTop50_hgnc/all_z_scores.Rdata")

unlist_z_scores_TAD <- unlist(lapply(all_z_scores, function(x) x[["z_scores_TAD"]]))
unlist_z_scores_TAD <- unname(abs(unlist_z_scores_TAD))

unlist_z_scores_adjTAD <- unlist(lapply(all_z_scores, function(x) x[["z_scores_adjTAD"]]))
unlist_z_scores_adjTAD <- unname(abs(unlist_z_scores_adjTAD))

outFile <- file.path(outFold, "cmp_density_TAD_adjTAD_log10.svg")
svg(outFile, height = 7, width = 10)
plot_multiDens(list(
  tad = log10(unlist_z_scores_TAD),
  adj_tad = log10(unlist_z_scores_adjTAD)
),
my_xlab = "abs(z-scores(log2FPKM)) [log10]",
plotTit = "abs(z-scores(log2FPKM))"
)
foo <- dev.off()
cat(paste0("... written: ", outFile, "\n"))
...
svg(outFile, height = 7, width = 10)
plot_multiDens(list(
  tad = unlist_z_scores_TAD,
  adj_tad = unlist_z_scores_adjTAD
),
my_xlab = "abs(z-scores(log2FPKM))",
plotTit = "abs(z-scores(log2FPKM))"
)
foo <- dev.off()
cat(paste0("... written: ", outFile, "\n"))
...
dataframeDT <- data.frame(
  values = c(unlist_z_scores_TAD, unlist_z_scores_adjTAD),
  type = c(rep("TAD", length(unlist_z_scores_TAD)), rep("adjTAD", length(unlist_z_scores_adjTAD)))
)
