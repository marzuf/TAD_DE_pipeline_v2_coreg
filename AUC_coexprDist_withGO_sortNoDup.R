startTime <- Sys.time()
cat(paste0("> Rscript AUC_coexprDist_withGO_sortNoDup.R\n"))
...
# Rscript AUC_coexprDist_withGO_sortNoDup.R TCGAcrc_msi_mss  # running

# Rscript AUC_coexprDist_withGO_sortNoDup <dataset> 
...
outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "AUC_COEXPRDIST_WITH_GO_SORTNODUP",  paste0(curr_dataset))
...
# mycols <- c("sameTAD" ="darkorange1" , "diffTAD"="darkslateblue",  "sameGO+sameTAD"="violetred1", "sameGO+diffTAD" = "lightskyblue")
...
sameGOsameTADcol <- mycols["same GO + same TAD"]
sameGOdiffTADcol <- mycols["same GO + diff. TAD"]
...
sameGO_sameTAD_DT <- allData_dt[allData_dt$sameGO == 1 & allData_dt$sameTAD == 1 ,c("gene1", "gene2", "coexpr", "dist",  "dist_kb")]
...
sameGO_diffTAD_DT <- allData_dt[allData_dt$sameGO == 1 & allData_dt$sameTAD == 0 ,c("gene1", "gene2",  "coexpr", "dist", "dist_kb")]
...
stopifnot(is.numeric(sameGO_sameTAD_DT$dist[1]))
stopifnot(is.numeric(sameGO_sameTAD_DT$coexpr[1]))
stopifnot(is.numeric(sameGO_diffTAD_DT$dist[1]))
stopifnot(is.numeric(sameGO_diffTAD_DT$coexpr[1]))
...
if(fitMeth == "loess") {
  ...
  smooth_vals_sameGOsameTAD <- predict(loess(coexpr ~ dist, data = sameGO_sameTAD_DT), sort(sameGO_sameTAD_DT$dist))
  smooth_vals_sameGOdiffTAD <- predict(loess(coexpr ~ dist, data = sameGO_diffTAD_DT), sort(sameGO_diffTAD_DT$dist))
  
  auc_sameGOdiffTAD_obsDist <- auc(x = sort(sameGO_diffTAD_DT$dist), y = smooth_vals_sameGOdiffTAD)
  auc_sameGOsameTAD_obsDist <- auc(x = sort(sameGO_sameTAD_DT$dist), y = smooth_vals_sameGOsameTAD)
  
  outFile <- file.path(outFold, paste0("sameGOsameTAD_sameGOdiffTAD_loessFit_originalDist", ".", plotType))
  do.call(plotType, list(outFile, height = myHeight, width = myWidth))
  plot(NULL,
       xlim = range(allData_dt$dist), 
       ylim = range(c(smooth_vals_sameGOsameTAD, smooth_vals_sameGOdiffTAD)),
       # xlab="", 
       # ylab="",
       xlab=my_xlab, 
       ylab=my_ylab,
       main=paste0(curr_dataset, ": coexpr ~ dist loess fit"))
  mtext(text = "observed distance values", side = 3)
  lines( x = sort(sameGO_sameTAD_DT$dist), y = smooth_vals_sameGOsameTAD, col = sameGOsameTADcol)
  lines( x = sort(sameGO_diffTAD_DT$dist), y = smooth_vals_sameGOdiffTAD, col = sameGOdiffTADcol)
  legend("topright", 
         legend=c(paste0("sameGOsameTAD\n(AUC=", round(auc_sameGOsameTAD_obsDist, 2), ")"), 
                  paste0("sameGOdiffTAD\n(AUC=", round(auc_sameGOdiffTAD_obsDist, 2))),  
         col = c(sameGOsameTADcol, sameGOdiffTADcol),
         lty=1,
         bty = "n")
  ...
  # PREDICT WITH DISTANCE VECTOR
  smooth_vals_sameGOsameTAD_distVect <- predict(loess(coexpr ~ dist, data = sameGO_sameTAD_DT), distVect)
  smooth_vals_sameGOdiffTAD_distVect <- predict(loess(coexpr ~ dist, data = sameGO_diffTAD_DT), distVect)
  
  auc_sameGOdiffTAD_distVect <- auc(x = distVect, y = smooth_vals_sameGOdiffTAD_distVect)
  auc_sameGOsameTAD_distVect <- auc(x = distVect, y = smooth_vals_sameGOsameTAD_distVect)
  
  sameGOdiffTAD_mod <- loess(coexpr ~ dist, data = sameGO_diffTAD_DT)
  outFile <- file.path(outFold, "sameGOdiffTAD_mod.Rdata")
  save(sameGOdiffTAD_mod, file = outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  sameGOdiffTAD_obsDist <- sameGO_diffTAD_DT$dist
  outFile <- file.path(outFold, "sameGOdiffTAD_obsDist.Rdata")
  save(sameGOdiffTAD_obsDist, file = outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  outFile <- file.path(outFold, "auc_sameGOdiffTAD_distVect.Rdata")
  save(auc_sameGOdiffTAD_distVect, file = outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  outFile <- file.path(outFold, "auc_sameGOsameTAD_distVect.Rdata")
  save(auc_sameGOsameTAD_distVect, file = outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  outFile <- file.path(outFold, "smooth_vals_sameGOsameTAD_distVect.Rdata")
  save(smooth_vals_sameGOsameTAD_distVect, file = outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  outFile <- file.path(outFold, "smooth_vals_sameGOdiffTAD_distVect.Rdata")
  save(smooth_vals_sameGOdiffTAD_distVect, file = outFile)
  cat(paste0("... written: ", outFile, "\n"))
  
  outFile <- file.path(outFold, paste0("sameGOsameTAD_sameGOdiffTAD_loessFit_vectDist.", plotType))
  do.call(plotType, list(outFile, height = myHeight, width = myWidth))
  plot(NULL,
       xlim = range(distVect), 
       ylim = range(c(na.omit(smooth_vals_sameGOsameTAD_distVect), na.omit(smooth_vals_sameGOdiffTAD_distVect))),
       # xlab="", 
       # ylab="",
       xlab=my_xlab,
       ylab=my_ylab,
       main=paste0(curr_dataset, ": coexpr ~ dist loess fit"))
  mtext(text = paste0("distance values seq from 0 to ", distLimit, " (# points = ", nbrLoessPoints, ")"), side = 3)
  lines( x = distVect, y = smooth_vals_sameGOsameTAD_distVect, col = sameGOsameTADcol)
  lines( x = distVect, y = smooth_vals_sameGOdiffTAD_distVect, col = sameGOdiffTADcol)
  legend("topright", 
         legend=c(paste0("sameGOsameTAD\n(AUC=", round(auc_sameGOsameTAD_distVect, 2), ")"), 
                  paste0("sameGOdiffTAD\n(AUC=", round(auc_sameGOdiffTAD_distVect, 2))), 
         col = c(sameGOsameTADcol, sameGOdiffTADcol),
         lty=1,
         bty = "n")
  ...
  ################################################################ 
  outFile <- file.path(outFold, paste0("sameTAD_diffTAD_sameGOsameTAD_sameGOdiffTAD_loessFit_vectDist.", plotType))
  do.call(plotType, list(outFile, height = myHeight, width = myWidth))
  plot(NULL,
       xlim = range(distVect), 
       ylim = range(c(na.omit(smooth_vals_sameTAD_distVect), 
                      na.omit(smooth_vals_sameGOsameTAD_distVect),
                      na.omit(smooth_vals_diffTAD_distVect),
                      na.omit(smooth_vals_sameGOdiffTAD_distVect))),
       # xlab="", 
       # ylab="",
       xlab=my_xlab,
       ylab=my_ylab,
  ...
  legend("topright", 
         legend=c(paste0("sameTAD\n(AUC=", round(auc_sameTAD_distVect, 2), ")"), 
                  paste0("diffTAD\n(AUC=", round(auc_diffTAD_distVect, 2)), 
                   paste0("sameGOsameTAD\n(AUC=", round(auc_sameGOsameTAD_distVect, 2), ")"), 
                   paste0("sameGOdiffTAD\n(AUC=", round(auc_sameGOdiffTAD_distVect, 2))), 
         col = c(sameTADcol, diffTADcol,
                 sameGOsameTADcol, sameGOdiffTADcol),
         lty=1,
         bty = "n")
  ...
  auc_values <- list(
    ...
    auc_sameGOdiffTAD_distVect = auc_sameGOdiffTAD_distVect,
    auc_sameGOsameTAD_distVect = auc_sameGOsameTAD_distVect,
    auc_ratio_sameGO_same_over_diff_distVect = auc_sameGOsameTAD_distVect/auc_sameGOdiffTAD_distVect,
    auc_sameGOdiffTAD_obsDist = auc_sameGOdiffTAD_obsDist,
    auc_sameGOsameTAD_obsDist = auc_sameGOsameTAD_obsDist,
    auc_ratio_sameGO_same_over_diff_obsDist = auc_sameGOsameTAD_distVect/auc_sameGOdiffTAD_obsDist
