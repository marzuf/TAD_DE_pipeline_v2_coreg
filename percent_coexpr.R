...
# Rscript percent_coexpr.R <dataset> 
# Rscript percent_coexpr.R TCGAcrc_msi_mss
...
outFold <- file.path(
  setDir, 
  "/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_TopDom/PERCENT_COEXPR",
  paste0("PERCENT_COEXPR_", distLimit/1000, "kb_", nSepMax, "GenesSep_", coexprThresh), curr_dataset)  
...
allData_dt_withPos$nGeneSep <- abs(allData_dt_withPos$nGene2 - allData_dt_withPos$nGene1) - 1 


# plot only the coexpressed -> positive coexpression value
plotDT <- allData_dt_withPos

plotDT_all <- allData_dt_withPos[allData_dt_withPos$nGeneSep <= nSepMax,]
plotDT_all <- aggregate(sameTAD ~ nGeneSep, FUN=length, data=plotDT_all)
...
plotDT_all_posCorr <- allData_dt_withPos[allData_dt_withPos$nGeneSep <= nSepMax,]
plotDT_all_posCorr_agg <- aggregate(coexpr ~ nGeneSep, FUN=function(x) sum(as.numeric(x > coexprThresh)), data=plotDT_all_posCorr)
...
plotDT_diffTAD_all <- allData_dt_withPos[allData_dt_withPos$sameTAD == 0 & allData_dt_withPos$nGeneSep <= nSepMax,]
plotDT_diffTAD_all <- aggregate(sameTAD ~ nGeneSep, FUN=length, data=plotDT_diffTAD_all)
...
plotDT_diffTAD <- allData_dt_withPos[allData_dt_withPos$sameTAD == 0  & allData_dt_withPos$nGeneSep <= nSepMax,]
plotDT_diffTAD_posCorr_agg <- aggregate(coexpr ~ nGeneSep, data=plotDT_diffTAD, FUN=function(x) sum(as.numeric(x > coexprThresh)))
...
plotDT_diffTAD_negCorr_agg <- aggregate(coexpr ~ nGeneSep, data=plotDT_diffTAD, FUN=function(x) sum(as.numeric(x <= coexprThresh)))
...
plotDT_sameTAD <- allData_dt_withPos[allData_dt_withPos$sameTAD == 1 & allData_dt_withPos$nGeneSep <= nSepMax,]
plotDT_sameTAD_posCorr_agg <- aggregate(coexpr ~ nGeneSep, data=plotDT_sameTAD, FUN=function(x) sum(as.numeric(x > coexprThresh)))
...
plotDT_sameTAD_negCorr_agg <- aggregate(coexpr ~ nGeneSep, data=plotDT_sameTAD, FUN=function(x) sum(as.numeric(x <= coexprThresh)))
...
plotDT_sameTAD_all_agg <- aggregate(coexpr ~ nGeneSep, data=plotDT_sameTAD, FUN=length)
...
plotDT_final <- merge(merge(merge(merge(merge(merge(plotDT_all, plotDT_diffTAD_all, by="nGeneSep"), plotDT_sameTAD_all_agg, by="nGeneSep"),
                plotDT_sameTAD_negCorr_agg, by="nGeneSep"), plotDT_sameTAD_posCorr_agg, by ="nGeneSep"),
                plotDT_diffTAD_negCorr_agg, by ="nGeneSep"), plotDT_diffTAD_posCorr_agg, by = "nGeneSep")
...
plotDT_final$nGeneSep <- factor(plotDT_final$nGeneSep, levels=as.character(0:nSepMax))

plotDT_m <- melt(plotDT_final[,c("nGeneSep", "redBars", "greyBars")], id="nGeneSep")

corrBarPlot <- ggplot(plotDT_m, aes(x=nGeneSep, y=value, fill=variable)) +
  ggtitle("Percentage of correlated gene pairs", subtitle = paste0(curr_dataset, " - coexpr. ", coexprThresh))+
  geom_bar(stat="identity", position="dodge")+
  scale_y_continuous(name = "% of genes in same TAD")+
  # scale_x_continuous(name = "# of genes separation", breaks = c(0:nSepMax), labels=as.character(c(0:nSepMax)))+
  scale_x_discrete(name = "# of genes separation", labels=c(0:nSepMax))+
