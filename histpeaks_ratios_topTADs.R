...
# => compare to v0:
# - Fisher test (pval and odds in the outputed table)
# - add FC for the ratioBin and the meanCount in the outputed table (MSI/MSS) (and rank according to FC ratioBin)

source("coreg_utils.R")

# Rscript histpeaks_ratios_topTADs.R
cat(paste0("> START ", "histpeaks_ratios_topTADs.R",  "\n"))
...
if(buildTable) {
  allHist_topTADs_histPeaks_DT <- foreach(histMark = all_hist_marks, .combine='rbind') %do% {
    ...
    topTADs_histPeaks_DT <- foreach(curr_tad = topTADs, .combine='rbind') %do% {
      ...
      # subDT=tmpBin_DT[tmpBin_DT$peak_name=="peak7024",]
      binRatio_DT <- do.call(rbind, by(tmpBin_DT, tmpBin_DT$peak_name, function(subDT) {
        ...
        cond1_peak <- sum(subDT$value[subDT$status == cond1] == 1)
        cond1_noPeak <- sum(subDT$value[subDT$status == cond1] == 0)
        cond2_peak <- sum(subDT$value[subDT$status == cond2] == 1)
        cond2_noPeak <- sum(subDT$value[subDT$status == cond2] == 0)
        
        testMatrix <- matrix(c(cond1_peak, cond2_peak,
                               cond1_noPeak, cond2_noPeak), byrow=T, nrow=2)  
        
        stopifnot(sum(testMatrix) == nSamplesCond1+nSamplesCond2)
        ...
        c(
          ...
          binFT_pval = as.numeric(ft$p.value),
          binFT_odds = as.numeric(ft$estimate)
          )
      }))
      ...
      # cat("... Compute Fisher tests for the peaks \n")
      ...
      final_binRatio_DT$ratioBin_FC <- final_binRatio_DT$ratioBinCond1/final_binRatio_DT$ratioBinCond2
      ...
      final_countRatio_DT$meanCount_FC <- final_countRatio_DT$meanCountCond1/final_countRatio_DT$meanCountCond2
      ...
      write_DT <- all_DT[,c("peak_name", "peak_chromo", "peak_start", "peak_end", 
                            paste0("ratioBin_", cond1), paste0("ratioBin_", cond2), "ratioBin_FC",
                            "binFT_pval", "binFT_odds",
                            paste0("meanCount_", cond1), paste0("meanCount_", cond2), "meanCount_FC")]
      
      write_DT[, paste0("ratioBin_", cond1)] <- round(write_DT[, paste0("ratioBin_", cond1)], 4)
      write_DT[, paste0("ratioBin_", cond2)] <- round(write_DT[, paste0("ratioBin_", cond2)], 4)
      write_DT[, paste0("meanCount_", cond1)] <- round(write_DT[, paste0("meanCount_", cond1)], 4)
      write_DT[, paste0("meanCount_", cond2)] <- round(write_DT[, paste0("meanCount_", cond2)], 4)
      write_DT[, paste0("binFT_pval")] <- round(write_DT[, paste0("binFT_pval")], 4)
      write_DT[, paste0("binFT_odds")] <- round(write_DT[, paste0("binFT_odds")], 4)
      
      
      write_DT1 <- write_DT
      # write_DT1 <- write_DT1[order(write_DT1[, paste0("ratioBin_", cond1)], write_DT1[, paste0("meanCount_", cond1)], decreasing=TRUE),]
      write_DT1 <- write_DT1[order(write_DT1[, paste0("ratioBin_FC")], write_DT1[, paste0("meanCount_FC")], decreasing=TRUE),]
							
      write_DT1[, paste0("ratioBin_", cond1)] <- round(write_DT1[, paste0("ratioBin_", cond1)], 4)
      write_DT1[, paste0("ratioBin_", cond2)] <- round(write_DT1[, paste0("ratioBin_", cond2)], 4)

      write_DT1[, paste0("meanCount_", cond1)] <- round(write_DT1[, paste0("meanCount_", cond1)], 4)
      write_DT1[, paste0("meanCount_", cond2)] <- round(write_DT1[, paste0("meanCount_", cond2)], 4)

      write_DT1[, paste0("ratioBin_FC")] <- round(write_DT1[, paste0("ratioBin_FC")], 4)
      write_DT1[, paste0("meanCount_FC")] <- round(write_DT1[, paste0("meanCount_FC")], 4)

      write_DT1[, paste0("binFT_pval")] <- round(write_DT1[, paste0("binFT_pval")], 4)
      write_DT1[, paste0("binFT_odds")] <- round(write_DT1[, paste0("binFT_odds")], 4)

      write_DT1$histMark <- histMark
      write_DT1 <- write_DT1[,c("histMark", colnames(write_DT1)[colnames(write_DT1) != "histMark"]) ]      
