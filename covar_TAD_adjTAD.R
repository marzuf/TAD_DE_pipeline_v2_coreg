startTime <- Sys.time()
cat(paste0("> Rscript covar_TAD_adjTAD.R\n"))
...
# Rscript covar_TAD_adjTAD.R
# Rscript covar_TAD_adjTAD.R TCGAcrc_msi_mss 50 hgnc
#  Rscript covar_TAD_adjTAD.R <dataset> <nTADs> <family>
...
#outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "COVAR_TAD_adjTAD",  paste0(curr_dataset, "_nTop", nTopTADs, "_", familyData))
outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "COVAR_TAD_adjTAD",  paste0(curr_dataset))
system(paste0("mkdir -p ", outFold))

logFile <- file.path(outFold, paste0("covar_TAD_adjTAD.txt"))  
...
all_covar_scores <- foreach(i_tad = seq_len(nrow(TADpos_DT))) %dopar% {  
  ...
  covar_tad <- cov(t(log2fpkm_tad))
  covar_adjTad <- cov(t(log2fpkm_adjTad))
  
  covar_TAD <-  covar_tad[lower.tri(covar_tad)]
  covar_adjTAD <- covar_adjTad[lower.tri(covar_adjTad)]

  list(covar_TAD = covar_TAD,
       covar_adjTAD = covar_adjTAD
       )  
}

names(all_covar_scores) <- TADpos_DT$region

outFile <- file.path(outFold, "all_covar_scores.Rdata")
save(all_covar_scores, file = outFile)
cat(paste0("... written: ", outFile, "\n"))

# load("/media/electron/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_TopDom/COVAR_TAD_adjTAD/TCGAcrc_msi_mss_nTop50_hgnc/all_covar_scores.Rdata")

unlist_covar_TAD <- unlist(lapply(all_covar_scores, function(x) x[["covar_TAD"]]))
unlist_covar_TAD <- unname(abs(unlist_covar_TAD))

unlist_covar_adjTAD <- unlist(lapply(all_covar_scores, function(x) x[["covar_adjTAD"]]))
unlist_covar_adjTAD <- unname(abs(unlist_covar_adjTAD))

outFile <- file.path(outFold, "cmp_density_TAD_adjTAD_log10.svg")
svg(outFile, height = 7, width = 10)
plot_multiDens(list(
  tad = log10(unlist_covar_TAD),
  adj_tad = log10(unlist_covar_adjTAD)
),
my_xlab = "lower tri. cov. log2FPKM [log10]",
plotTit = "gene pairwise cov. log2FPKM"
)
foo <- dev.off()
cat(paste0("... written: ", outFile, "\n"))
...
svg(outFile, height = 7, width = 10)
plot_multiDens(list(
  tad = unlist_covar_TAD,
  adj_tad = unlist_covar_adjTAD
),
my_xlab = "lower tri. cov. log2FPKM [log10]",
plotTit = "gene pairwise cov. log2FPKM"
)
...
dataframeDT <- data.frame(
  values = c(unlist_covar_TAD, unlist_covar_adjTAD),
  type = c(rep("TAD", length(unlist_covar_TAD)), rep("adjTAD", length(unlist_covar_adjTAD)))
)
dataframeDT$values <- log10(dataframeDT$values)
dataframeDT$type <- factor(dataframeDT$type, levels = c("adjTAD", "TAD"))

my_comparisons <- list( c("TAD", "adjTAD") )

violinP <- ggviolin(dataframeDT, x = "type", 
                    y = "values", 
                    fill = "type",
                    legend.title = "",
                    # yscale = "log10",
                    ...
                    xlab ="",
                    ylab ="gene log2FPKM covar [log10]",
