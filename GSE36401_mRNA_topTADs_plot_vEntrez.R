...
# Rscript GSE36401_mRNA_topTADs_plot_vEntrez.R
...
outFold <- file.path(setDir, "/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_coreg/h3k27ac_msi_mss_data/GSE_data/GSE36401_Akhtar-Zaidi2012/GSE36401_mRNA_TOPTADs_BOXPLOT_vEntrez", curr_dataset)
...
for(tad in top_tads) {
  ...
  if(nrow(curr_df) == 0) {
    cat("! No entrezID mapping to topTAD: ", tad, "!\n")
    next
  }
  ...
  for(countType in plot_countType) {
    ...
    outFile <- file.path(outFold, paste0(tad, "_mRNA", countType, "_vEntrez.", plotType))
