startTime <- Sys.time()
cat(paste0("> Rscript prep_all_genes_data.R\n"))
...
outFold <- file.path(setDir, paste0("/mnt/ed4/marie/scripts/TAD_DE_pipeline_v2_", caller), "PREP_ALL_GENES_TMP")
...
for(i_fam in all_familyData) {
  ...
  pairsDT <- foreach(i_1 = 1:(nrow(gene2tadDT)-1), .combine='rbind') %:%
